...
import static com.enliple.pudding.timepicker.TimePickerDialog.HOUR_INDEX;
import static com.enliple.pudding.timepicker.TimePickerDialog.MINUTE_INDEX;
...
class DefaultTimepointLimiter implements TimepointLimiter {
    ...
    void setMinTime(@androidx.annotation.NonNull Timepoint minTime) {
        if(mMaxTime != null && minTime.compareTo(mMaxTime) > 0)
            throw new IllegalArgumentException("Minimum time must be smaller than the maximum time");
        mMinTime = minTime;
    }

    void setMaxTime(@androidx.annotation.NonNull Timepoint maxTime) {
        if(mMinTime != null && maxTime.compareTo(mMinTime) < 0)
            throw new IllegalArgumentException("Maximum time must be greater than the minimum time");
        mMaxTime = maxTime;
    }

    void setSelectableTimes(@androidx.annotation.NonNull Timepoint[] selectableTimes) {
        mSelectableTimes.addAll(Arrays.asList(selectableTimes));
        exclusiveSelectableTimes = getExclusiveSelectableTimes(mSelectableTimes, mDisabledTimes);
    }

    void setDisabledTimes(@androidx.annotation.NonNull ArrayList<Timepoint> disabledTimes) {
        mDisabledTimes.addAll(disabledTimes);
        exclusiveSelectableTimes = getExclusiveSelectableTimes(mSelectableTimes, mDisabledTimes);
    }

    void setDisabledTimes(@androidx.annotation.NonNull Timepoint[] disabledTimes) {
    ...
    @androidx.annotation.NonNull
    private TreeSet<Timepoint> getExclusiveSelectableTimes(@androidx.annotation.NonNull TreeSet<Timepoint> selectable, @androidx.annotation.NonNull TreeSet<Timepoint> disabled) {
    ...
    @Override
    public boolean isOutOfRange(@androidx.annotation.Nullable Timepoint current, int index, @androidx.annotation.NonNull Timepoint.TYPE resolution) {
    ...
    public boolean isOutOfRange(@androidx.annotation.NonNull Timepoint current) {
    ...
    Timepoint roundToNearest(@androidx.annotation.NonNull Timepoint time, @androidx.annotation.Nullable Timepoint.TYPE type, @androidx.annotation.NonNull Timepoint.TYPE resolution) {
    ...
    private Timepoint searchValidTimePoint(@androidx.annotation.NonNull Timepoint time, @androidx.annotation.Nullable Timepoint.TYPE type, @androidx.annotation.NonNull Timepoint.TYPE resolution) {
