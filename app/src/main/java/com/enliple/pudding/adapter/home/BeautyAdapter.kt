 ...
 */
class BeautyAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder> {
    companion object {
    ...
    fun setFirstCategory(f_category: String) {
        this.firstCategory = f_category
    }
    ...
    fun setItem(item: API114.VideoItem, position: Int) {
        if (mItems != null) {
    ...
    private fun onBindEvent(holder: BeautyEventHolder) {
        if(mEventItems.size > 0) {
            ...
            holder.itemView.viewPager.offscreenPageLimit = mEventItems.size
    ...
    private fun onBindCategory(holder: BeautyCategoryHolder) {
        if (!isCategoryClick) {
            val json = DBManager.getInstance(mContext).get(NetworkHandler.getInstance(mContext).getKey(NetworkApi.API81.toString(), "select_union", "", "beauty"))
