...
import kotlinx.android.synthetic.main.adapter_home_shopping_hot_item.view.*

/**
 * Created by Kim Joonsung on 2019-06-26.
 */
class HotShoppingAdapter : RecyclerView.Adapter<HotShoppingAdapter.HotLiveHolder> {
    private val items: MutableList<API114.HotDataItem> = mutableListOf()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HotLiveHolder {
        return HotLiveHolder(LayoutInflater.from(parent.context).inflate(R.layout.adapter_home_shopping_hot_item, parent, false))
    }
...
/*            // 기능제외
    ...
    fun setItems(items: List<API114.HotDataItem>) {
        this.items.clear()
        if (items.isNotEmpty()) {
    ...
    interface Listener {
        fun onHotItemClicked(item: API114.HotDataItem, position: Int)
    }
}