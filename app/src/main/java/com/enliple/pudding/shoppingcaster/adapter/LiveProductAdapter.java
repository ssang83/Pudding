...
public class LiveProductAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    boolean isSample = false, isPromotion = false;
    private String firstCategory, secondCategory;
    public LiveProductHeaderViewHolder headerViewHolder;
    public LiveProductItemViewHolder itemViewHolder;
    ...
    public LiveProductAdapter(Context mContext, boolean isSample, boolean isPPromotion) {
...
//        Logger.e("viewWidth :: " + viewWidth);
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_HEADER) {
            ...
            headerViewHolder = new LiveProductHeaderViewHolder(mContext, view);
            return headerViewHolder;
        } else {
            View view = LayoutInflater.from(mContext).inflate(R.layout.live_product_item, parent, false);
            itemViewHolder = new LiveProductItemViewHolder(view);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof LiveProductHeaderViewHolder) {
            bindLiveProductHeaderHolder((LiveProductHeaderViewHolder) holder);
        } else {
            bindLiveProductItemViewHolder((LiveProductItemViewHolder) holder, position - 1);
        }
    }
     ...
     * @param holder
     */
    private void bindLiveProductHeaderHolder(final LiveProductHeaderViewHolder holder) {
...
//                    getList(true, holder);
    ...
    /**
     * LiveProduct 아이템 ViewHolder를 Binding
     *
     * @param holder
     * @param position
     */
    private void bindLiveProductItemViewHolder(final LiveProductItemViewHolder holder, final int position) {
        Logger.e("bindLiveProductItemViewHolder aassdd");
...
//        }
    ...
    public void setHeaderData(ArrayList<CategoryItem> subArray) {
        Logger.e("setHeaderData aassdd");
        this.subArray = subArray;
//        notifyItemChanged(0);
        notifyDataSetChanged();
    }

    public void setHeaderData(ArrayList<CategoryItem> subArray, LiveProductHeaderViewHolder holder) {
        this.subArray = subArray;
        for (int i = 0; i < subArray.size(); i++) {
            Logger.e("subArray. val :: " + subArray.get(i).getCategoryName());
        }
        notifyDataSetChanged();
        holder.headerAdapter.notifyDataSetChanged();
    }

//    public void addItems(ArrayList<API70.ProductItem> items) {
...
//    }
    ...
    public void setCategoryData(ArrayList<CategoryItem> items, LiveProductHeaderViewHolder holder) {
        setHeaderData(items, holder);
    }

//    private void changeCategory(LiveProductHeaderViewHolder holder, String subCategory, boolean isMainCategoryChanged) {
//        String selectedMainCategory = holder.headerAdapter.getSelectedCategory().getCategoryName();
//        // TODO :: selectedMainCategory와 subCategory를 이용해 통신, 통신성공 후 sub category arry 받아와서 setSubCategory 하도록 변경 해야함
//        Logger.e("CATEGORY MAIN :: " + selectedMainCategory + " , CATEGORY SUB :: " + subCategory);
//        setSubCategory(holder, isMainCategoryChanged);
//    }
...
//    void onSendQnA(String contents);
        ...
        void onCategoryClicked(boolean isMainClicked, String categoryId, LiveProductHeaderViewHolder holder);

        void onSortClicked(String order);
    }

    public static class LiveProductHeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        public LiveProductHeaderViewHolder(Context context, View itemView) {
            super(itemView);
            subCategoryLayer = (LinearLayout) itemView.findViewById(R.id.subCategoryLayer);
            headerRecyclerView = (RecyclerView) itemView.findViewById(R.id.recyclerHeader);
...
//            firstCategoryArray = DatabaseAPI.INSTANCE.getFirstCategorysWithoutPopularity(context.getContentResolver());
    ...
    static class LiveProductItemViewHolder extends RecyclerView.ViewHolder {
        ...
        public LiveProductItemViewHolder(View itemView) {
            super(itemView);
    ...
    private void setSubCategory(LiveProductHeaderViewHolder holder) {
