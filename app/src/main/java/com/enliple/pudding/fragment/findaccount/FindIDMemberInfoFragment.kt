...
/**
 * 아이디 찾기 - 회원정보찾기 방식 메인 Fragment
 * @author hkcha
 * @since 2018.08.16
 */
class FindIDMemberInfoFragment : AbsBaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        textViewGuide.text = "* 가입 당시 입력한 이메일 주소를 입력해주세요."
        ...
        if (cellPhoneFragment == null) {
            cellPhoneFragment = MemberCellPhoneFragment().apply {
                arguments = Bundle().apply {
                    putInt(MemberCellPhoneFragment.BUNDLE_EXTRA_KEY_CALL_MODE, MemberCellPhoneFragment.BUNDLE_EXTRA_VALUE_FIND_ID)
                }
            }
        }
    ...
    private fun replaceFindEmailFragment() {
        ...
        if (emailFragment == null) {
            emailFragment = MemberEmailFragment().apply {
                arguments = Bundle().apply {
                    putInt(MemberEmailFragment.BUNDLE_EXTRA_KEY_CALL_MODE, MemberEmailFragment.BUNDLE_EXTRA_VALUE_FIND_ID)
                }
            }
        }
