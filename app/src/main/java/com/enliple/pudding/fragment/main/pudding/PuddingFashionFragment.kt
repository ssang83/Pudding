...
import com.enliple.pudding.adapter.home.FashionAdapter
 ...
 */
class PuddingFashionFragment : androidx.fragment.app.Fragment(), FashionAdapter.Listener {

    private lateinit var mAdapter: FashionAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        mAdapter = FashionAdapter(context!!).apply {
            setListener(this@PuddingFashionFragment)
        }
    ...
    override fun setUserVisibleHint(isVisibleToUser: Boolean) {
        ...
        if (isVisibleToUser) {
            if (mIsFirst) {
                Logger.e("first fashion")
                NetworkBus(NetworkApi.API81.name, "select_union", "", "fashion").let {
    ...
    override fun onCategoryClick(categoryId: String) {
        ...
        if ("all" == categoryId) {
            EventBus.getDefault().post(NetworkBus(NetworkApi.API114.name, "fashion", "1", "", fashionOrder))
        } else {
            EventBus.getDefault().post(NetworkBus(NetworkApi.API114.name, "fashion", "1", categoryId, fashionOrder))
        }
    }
    ...
    override fun onPopularityOrderClicked(order: String) {
        ...
        EventBus.getDefault().post(NetworkBus(NetworkApi.API114.name, "fashion", "1", mCategoryId, order))
    }
    ...
    fun onMessageEvent(data: NetworkBusResponse) {
        if(data.arg1.startsWith("POST/products/") && data.arg1.endsWith("wish")) {
            ...
            if(mIsVisibleToUser && (parentFragment as PuddingHomeTabFragment).userVisibleHint) {
                val api81 = NetworkHandler.getInstance(activity!!).getKey(NetworkApi.API81.toString(), "select_union", "", "fashion")
                if (data.arg1.startsWith("GET/mui/main/fashion?page=")) {
    ...
    private fun refresh() {
        if (mIsVisibleToUser && (parentFragment as PuddingHomeTabFragment)?.userVisibleHint) {
            ...
            EventBus.getDefault().post(NetworkBus(NetworkApi.API114.name, "fashion", "1", mCategoryId, fashionOrder))
        }
    }
...
//                }
                ...
                if ((lastVisibleItemPosition == totalItemCount) && !isEndOfData) {
                    ...
                    EventBus.getDefault().post(NetworkBus(NetworkApi.API114.name, "fashion", pageCount.toString(), mCategoryId, fashionOrder))
                }
            }
        }
    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if ( action == "likeChange") {
                ...
                if ( gubun == "run") {
                    if ( page == PuddingHomeTabFragment.PAGE_FASHION && ::mAdapter.isInitialized) {
                        mAdapter!!.runTimer()
                    }
                    ...
                    if (::mAdapter.isInitialized ) {
                        if ( page == PuddingHomeTabFragment.PAGE_FASHION ) {
                            mAdapter!!.runTimer()
                        } else {
                            mAdapter!!.cancelTimer()
                        }
