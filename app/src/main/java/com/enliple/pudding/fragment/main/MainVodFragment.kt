 ...
 * Main HOT category
 */
class MainVodFragment : VideoBaseFragment() {
    companion object {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mAdapter = MainListAdapter(activity!!, childFragmentManager!!, 3)
        ...
        //var bus = NetworkBus(NetworkApi.VOD0.name, "vod", "1")
        //EventBus.getDefault().post(NetworkBus("refresh_all_video"))
    }

    override fun handleNetworkResult(data: NetworkBusResponse) {
        var key = NetworkHandler.getInstance(context!!).getKey(NetworkApi.VOD0.toString(), "vod", "1")
    ...
    private fun getFragment(position: Int): MainVideoFragment? {
        if (mAdapter != null) {
            Logger.d("getCurrentFragment:" + flippableViewPager.currentItem)
